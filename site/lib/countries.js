// ISO 3166-1 alpha-2 codes + common English short names.
// Includes a few newsworthy non-UN-member entities (Taiwan, Vatican, Palestine, Kosovo).
...
// Curated by current traffic relevance — matches the worker's active trend markets
// (worker/trends.js COUNTRY_NAMES). Surfaced first so the most-likely-to-have-stories
// countries don't get lost in an alphabetical list of 190+.
export const POPULAR_CODES = ["US", "GB", "CA", "AU", "IN", "DE", "FR", "BR", "JP", "NG"];
...
export function popularCountries() {
  return POPULAR_CODES.map((code) => ({ ...BY_CODE.get(code) }));
}
